...
export async function startPairing() {
  // Check if keys already exist in storage (generated by offscreen/crypto.js)
  ...
  // Keys don't exist yet — the offscreen document generates them via libsodium.
  // Send a keepalive ping to wake the service worker + offscreen document.
  try {
    await chrome.runtime.sendMessage({ type: 'keepalive-ping' });
  } catch (_) {
    // SW might not be ready yet; that is OK — we poll below.
  }

  // Poll storage for up to 5 seconds waiting for offscreen-generated keys.
  for (let i = 0; i < 25; i++) {
    await new Promise(r => setTimeout(r, 200));
    stored = await chrome.storage.local.get(['deviceId', 'deviceKeys']);
    if (stored.deviceId && stored.deviceId.length >= 16 && stored.deviceKeys?.ed25519?.pk) {
      console.log('[Beam] Keys appeared after polling. deviceId:', stored.deviceId);
      return _buildQrData(stored);
    }
  }

  console.error('[Beam] Keys not generated after 5 seconds — offscreen may not be running');
  return null;
}
